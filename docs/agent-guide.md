...
- Obsidian import implementation is layered under `scripts/obsidian-import/`; `scripts/import-obsidian-blog.mjs` is only the compatibility entry. Source notes must provide explicit `note_id` or `笔记ID`; missing ids should be skipped with import errors and must not be silently derived from dates. Keep new cleanup rules inside the transform/plugin modules instead of re-coupling them into the entry file. Dataview is intentionally not evaluated yet; fenced Dataview blocks should become static placeholders with diagnostics unless a future task explicitly implements a safe query subset. Frontmatter tag policy lives in `scripts/obsidian-import/frontmatter/tags.mjs`; keep published tags small, flat, and stable, add aliases there deliberately, and use diagnostics for tags that should not become public site tags. The importer derives `type: writeup` from the normalized `writeup` tag and derives `domain` from normalized domain tags like `pwn`, `web`, `rev`, `crypto`, and `misc`; source notes tagged `汇编指令` are treated as `domain: rev`. Do not reintroduce old nested tag families like `arch/...`, `mitigation/...`, `tech/...`, or `note/...` as public tag whitelist entries.
