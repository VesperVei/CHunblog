...
- The current TheBrain-specific provider is `src/components/graph/graph-node-overlay.ts`. The current first version is intentionally narrow: a single-column panel showing only `graphLevel`, direct graphLevel edit actions, and a placeholder line `关系设置未实现`. Future layout-specific settings should extend this provider layer rather than re-coupling panel DOM/positioning into `graph-client.ts`.
